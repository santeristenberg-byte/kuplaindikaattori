...
UA = {"User-Agent": "Mozilla/5.0 (kuplaindikaattori; henkilokohtainen tutkimuskaytto)"}
...
def fetch_fred(sid: str) -> pd.Series:
    ...
    r = requests.get(FRED_CSV.format(id=sid), headers=UA, timeout=60)
    r.raise_for_status()
    df = pd.read_csv(io.StringIO(r.text))
    df = df.iloc[:, :2]
    df.columns = ["date", "value"]
    return to_monthly(pd.Series(df["value"].values, index=df["date"].values))
...
def fetch_shiller() -> pd.DataFrame:
    ...
    r = requests.get(SHILLER_XLS, headers=UA, timeout=120)
    r.raise_for_status()
    raw = pd.read_excel(io.BytesIO(r.content), sheet_name="Data", header=None)

    # Etsi CAPE-sarake otsikkoriveiltä (otsikko on kahdella rivillä)
    cape_col = None
    for i in range(min(12, len(raw))):
        row = raw.iloc[i].astype(str).str.strip()
        hits = [j for j, v in enumerate(row) if v == "CAPE"]
        if hits:
            cape_col, header_row = hits[0], i
            break
    if cape_col is None:
        raise RuntimeError("Shillerin taulukosta ei löytynyt CAPE-saraketta")
    ...
    idx = pd.to_datetime(pd.DataFrame({"year": year, "month": month, "day": 1})) + pd.offsets.MonthEnd(0)

    df = pd.DataFrame(
        {
            "P": pd.to_numeric(body.iloc[:, 1], errors="coerce").values,
            "E": pd.to_numeric(body.iloc[:, 3], errors="coerce").values,
            "CPI": pd.to_numeric(body.iloc[:, 4], errors="coerce").values,
            "CAPE": pd.to_numeric(body.iloc[:, cape_col], errors="coerce").values,
        },
        index=idx,
    )
    df = df[~df.index.duplicated(keep="last")].sort_index()
    if df["CAPE"].dropna().iloc[-1] < 3 or df["CAPE"].dropna().iloc[-1] > 100:
        raise RuntimeError("Shillerin CAPE-arvo epäuskottava — taulukon rakenne on ehkä muuttunut")
    return df
...
def fetch_finra_margin() -> pd.Series:
    ...
    try:
        r = requests.get(FINRA_URL, headers=UA, timeout=60)
        r.raise_for_status()
